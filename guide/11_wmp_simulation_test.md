...
带窗口运行还需要当前 Python 环境中的 `cv2`；本机复用 JetPack OpenCV 4.2.0。
...
- 已完成 Orin 延时测量：actor-only 帧满足 20 ms，但每五帧一次的 world-model 帧不满足；
- 待图形环境具备后：实际 MuJoCo viewer、depth_camera 渲染和闭环姿态表现；
- 未完成也不在本步骤执行：Orin D435i 实机采集、LowCmd、Sport Mode、真实机器人动作。

25W mode 3 下使用短测最优的 4 个 CPU 线程执行 500 帧，完整 pipeline 平均为
`8.175 ms`，但 world-model 更新帧平均 `31.009 ms`，100 个更新帧全部超过 20 ms。
因此不能用平均 `122.3 Hz` 宣称 WMP 已满足逐帧 50 Hz，也不能进入 WMP 实机阶段。
详见 `13_orin_policy_benchmark.md`。
