...
脚本按用途放入四个子目录，避免离线检查、仿真和实机入口混在一起：
...
`scripts/` 根目录不保留重复的启动脚本，运行时直接调用上述分类目录中的实际文件。项目文档统一使用这些分类路径；不要再使用旧的 `scripts/test_*.py` 或 `scripts/debug_*.py` 根目录路径。只有 `scripts/input/debug_unitree_remote.py` 和 `scripts/real/` 下的入口会访问机器人 DDS，`policy/` 与 `simulation/` 不应连接实机。
