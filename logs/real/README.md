...
本目录中的实验文件（JSONL、CSV、NPZ 和自动生成的 `_obs/` 目录）均由 `.gitignore` 忽略，
只保留本 README 在 Git 中。正式 `run_go2wwmp_unitree.py` 默认生成主 JSONL 和同名 `_obs/`
观测目录；观测格式、字段和读取方法见 `guide/22.5_go2wwmp_log_format.md`。
